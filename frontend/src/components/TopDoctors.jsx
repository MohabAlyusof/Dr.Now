  ...
  return (
    <div className="my-16">
      <div className="max-w-7xl mx-auto px-6 md:px-16 flex flex-col items-center gap-6 text-[#262626]">
        <h1 className="text-3xl font-semibold text-[#126A9C]">Top Doctors to Book</h1>
          ...
          {doctors.slice(0, 10).map((item, index) => (
            ...
            >
              ...
              <div className="p-4 text-center">
                <div className={`flex items-center justify-center gap-2 text-sm mb-2 ${item.available ? 'text-[#2F9650]' : 'text-gray-500'}`}>
                  <span className={`w-2 h-2 rounded-full ${item.available ? 'bg-[#2F9650]' : 'bg-gray-500'}`}></span>
                  <span>{item.available ? 'Available' : 'Not Available'}</span>
                </div>
                <p className="text-lg font-semibold text-[#262626]">{item.name}</p>
                <p className="text-gray-500 text-sm capitalize">{item.speciality}</p>
              </div>
            </div>
          ))}
        </div>
