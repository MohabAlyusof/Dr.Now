  ...
  return (
    <div className="px-6 md:px-16 max-w-7xl mx-auto text-[#262626]">
      <h2 className="text-center text-2xl pt-10 text-[#707070]">
        Browse Through Our <span className="text-gray-700 font-semibold">Doctors</span>
      </h2>
      ...
      <div className="flex flex-col sm:flex-row items-start gap-5 mt-10">
        ...
        <button 
          ...
          className={`py-2 px-4 border rounded-xl text-sm shadow-md transition-all sm:hidden ${showFilter ? 'bg-[#126A9C] text-white' : 'bg-white text-gray-700'}`}
        >
          Filters
        </button>
        ...
        <div className={`flex-col gap-4 text-sm text-gray-600 ${showFilter ? 'flex' : 'hidden sm:flex'}`}>
          {["General physician", "Gynecologist", "Dermatologist", "Pediatricians", "Neurologist", "Gastroenterologist"].map((spec, index) => (
            <p 
              ...
              className={`w-[94vw] sm:w-auto pl-3 py-2 pr-16 border border-gray-300 rounded-xl shadow-sm cursor-pointer transition-all hover:bg-[#f0f4ff] ${speciality === spec ? 'bg-[#E2E5FF] text-black font-semibold' : ''}`}
            >
              {spec}
            </p>
          ))}
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {filterDoc.map((item, index) => (
            ...
            >
              <img className="bg-[#EAEFFF] w-full h-56 object-cover" src={item.image} alt={item.name} />
